...
function saveData() {
    handSignData.push({
        pose: results.landmarks.map(flattenCoordinates)[0],
        label: selectedHandSign
    });
}

function exportData() {
    const dataStr = JSON.stringify(handSignData, null, 2);
    const dataUri = 'data:application/json;charset=utf-8,' + encodeURIComponent(dataStr);
    const exportFileDefaultName = 'handsign_data.json';
...
document.getElementById('handsignSelect').addEventListener('change', function(event) {
    selectedHandSign = event.target.value;
});
